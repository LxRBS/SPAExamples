...
/// 使用拼音防止命名冲突
#define fanwei (llvm::make_range)
#define beend(p) (fanwei((p)->begin(), (p)->end()))
...
/**
 * 将k,v加入到指定的unordered_map
 * map维护的(K, V的数组)
 * 如果m中没有k，就新创建一个数组，并把v放进去
 * 如果m中有k，就把v加在已有数组的后面 
*/
template<typename K, typename V>
void insert(const K &k, const V &v, std::unordered_map<K, std::vector<V> > &m){
    auto it = m.find(k);
    if(it==m.end()){
        m.insert(it, mp(k, std::vector<V>(1, v)));
    }else{

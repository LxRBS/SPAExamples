...
int main(int argc, char * argv[]){
    ...
    SignAnalyzer analyzer(svfModule, symbolTableInfo);
    analyzer.analyze();
    analyzer.report(out);
    out.close();    

    string vmAnaName("./"+prefix+"/"+prefix+"_valuemaintiananalysis.txt");
    ofstream out2(vmAnaName);
    ValueMaintainAnalyzer analyzer2(svfModule, symbolTableInfo);
    analyzer2.analyze();
    analyzer2.report(out2);
    out2.close();    
    return 0;
}